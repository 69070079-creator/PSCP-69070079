...
```text
หาจำนวนร้านที่เปิดในช่วงเวลาที่ไป check  
Input:
รับค่าจำนวนเต็ม num,check
รับค่าเปิด ปิดร้าน num บรรทัด
รับค่าเวลาที่ไป check จำนวน check ค่า

Output:
พิมพ์จำนวนร้่านที่เปิดใน check นั้น

Constraints:
input รับหลายค่าใน 1 บรรทัด เลยต้องใช้ .split() และ output ต้องพิม์ออกมา 1 บรรทัด 
```
...
```text
Step 1: รับค่าจำนวนเต็ม num,check
Step 2: ใช้ for loop range(num) รับค่าเปิด ปิดร้าน แล้วเพื่มใน list
Step 3: รับค่าเวลาที่ไป check จำนวน check ค่า
Step 4:  ใช้ for loop range(check) ตรวจว่ามีร้านเปิดไหม
Step 5: พิมพ์ค่าที่ได้ออกมา
```
...
```text
ไม่เหมือน ผมสร้างฟังก์ชันตรวจสอบต่ำแหน่ง เพื่อใช้ในการหาจำนวนร้านที่เปิดเพิ่มขึ้นมา
```
...
```text
เพื่อทดสอบว่าโปรแกรมสามารถใช้งานกับ test cases อื่นที่ไม่ใช่ simple test cases ได้ไหม(มีร้่านเปิด)
```
...
```text
เพื่อทดสอบว่าโปรแกรมสามารถใช้งานกับ test cases อื่นที่ไม่ใช่ simple test cases ได้ไหม(มีร้่านปิด)
```
...
```text
เพื่อทดสอบว่าโปรแกรมสามารถใช้งานกับ test cases อื่นที่ไม่ใช่ simple test cases ได้ไหม(มีร้่านเปิด และ ปิด)
```
...
```text
เขาช่วยแนะนำว่าควรเริ่มยังไง(ลองใช้ loop ในการการดู)
```

คุณยังทำอะไรด้วยตนเอง

```text
ผมลองผิดลองถูกตามวิธีที่พี่แนะนำ
```
