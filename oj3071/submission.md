...
Output:
พิมพ์  จำนวน x  ในช่วง [A , B] ที่หารด้วยจํานวนนับ d แล้วเหลือเศษ r

Constraints:
โจทย์ให้หาจํานวนนับ x ในช่วง [A , B] จำเป็นต้องใช้รูป เวลาใช้ for loop ตัวปิดอย่าง b ต้อง + 1 ด้วย
```
...
```text
Step 1: รับจำนวนเต็ม 4 บรรทัด
Step 1: สร้าง count  เพื่อเก็บค่า x 
Step 2: เข้าลูปหาจำนวน x ในช่วง (a,b+1)
Step 3: ตรวจสอบเงื่อนไข ถ้า % d == r ค่า count += 1
Step 4: ลูปเสร็จแล้ว พิมพ์ค่า count
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรก ใช้ for i in range (a, b + 1): หาค่าที่ % == r แล้วค่า count += 1 หลังลูปเสร็จพิมพ์ count
```
