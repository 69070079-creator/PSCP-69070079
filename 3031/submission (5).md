...
```text
โจทย์ให้รับค่าจำนวนเต็มสองตัว 1 บรรทัดและรับค่าพิกัด(x,y) i +1 บรรทัด เพื่อดูว่าในวินาทีที่เท่าไหร่ค่าจำนวนเต็มจะถึงพิกัด(x,y)
Input:
บรรทัดที่ 1:รับจำนวนเต็ม s แทนค่าพื้นที่
รับจำนวนเต็ม n แทนจำนวนคำร้อง
บรรทัดที่ i + 1(i <= n):รับค่าพิกัดบ้านผู้ให้คำร้อง

Output:
พิมพ์ว่าอีกกี่วินาที่(จำนวนเต็ม)ที่ s จะถึงพิกัด(x,y)ของผู้ให้คำร้อง

Constraints:
โจทย์ให้ค่าpi = 3.1416 มาในการคำนวนแต่ output เป็นจำนวนเต็ม ผมเลยพิจารณาใช้ math.ceil เพื่อปัดเศษทศนิยมที่อาจเกิดขึ้นระหว่างคำนวน
```
...
```text
Step 1: import math 
Step 2: รับจำนวนเต็ม s แทนค่าพื้นที่
รับจำนวนเต็ม n แทนจำนวนคำร้อง
Step 3: ใช้ for i in range(n): เพื่อตรวจเวลาที่จะถึงพิกัด(x,y)ของ n แต่ละคน 
Step 4: รับค่าพิกัด(x,y)
Step 5: ใช้สูตรระยะห่างกำลังสองเพื่อหาระยะห่างรห่างจุด (0,0)กับ(x,y) (dist^2 = x^2 + y^2)
Step 6: คำนวนหาเวลาที่จะถึงพิกัด(x,y)  t = math.ceil((dist_sq *PI) / s)
Step 7: พิมพ์เวลาที่ได้
```
...
```text
วิธีสุดท้ายไม่เหมือนกับแผนแรก เพราะ ผมเปลี่ยนจากใช้ for loop ไปใช้ while แทนเพราะติดปัญหา pep8  ส่วนจุดอื่นผมใช้โครงสร้างเดิมคือ คำนวนหาระยะห่างระหว่าง(0,0)กับ(x,y) แล้วนำค่าที่ได้ไปคำนวนหาเวลา และพิมพ์เวลาที่ได้ออกมา
```
...
```text
เพราะบ้านที่อยู่จุดเดียวกับพื่นที่เวลาที่จะควรไปถึงบ้านคือ 0 วินาที
```
...
```text
เพราะยิ่งพื้นที่เยอะเวลาควรยิ่งน้อยลง
```
...
```text
ทดสอบค่าพิกัดสูงสุดตามที่โจทย์กำหนด
```
